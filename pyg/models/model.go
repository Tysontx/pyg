...
type User struct {
	...
	Addresses  []*Address   `orm:"reverse(many)"` // 用户和地址一对多
...
func init() {
	...
	// 注册表结构
	orm.RegisterModel(new(User), new(Address), new(TpshopCategory), new(Goods), new(GoodsType), new(GoodsSKU), new(GoodsImage), new(IndexGoodsBanner), new(IndexTypeGoodsBanner), new(IndexPromotionBanner), new(OrderInfo), new(OrderGoods))
	// 跑起来
